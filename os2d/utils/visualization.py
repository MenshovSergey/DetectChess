...
def show_detections(boxes, image_to_show,
                    ...
                    lower=True, max_parabola_deviation=None):
    ...
    return show_annotated_image(img=image_to_show,
                         ...
                         area_threshold=cfg_visualization.area_threshold,
                         max_dets=cfg_visualization.max_detections,
                         showfig=False,
                         image_id=image_id,
                         lower=lower,
                         max_parabola_deviation=max_parabola_deviation)
...
def get_box_area(box):
    left = float(box.bbox_xyxy[0][0])
    top = float(box.bbox_xyxy[0][1])
    right = float(box.bbox_xyxy[0][2])
    bottom = float(box.bbox_xyxy[0][3])
...
def sqr_dist_between(pt1, pt2):
    return (pt1[0] - pt2[0]) * (pt1[0] - pt2[0]) + (pt1[1] - pt2[1]) * (pt1[1] - pt2[1])


def get_center(box):
    return [(box.bbox_xyxy[0][0] + box.bbox_xyxy[0][2]) / 2, (box.bbox_xyxy[0][1] + box.bbox_xyxy[0][3]) / 2]


def approx_dist_between_point_and_parabola(k, y0, width, point):
    return abs(y0 + k * (point[0] - width / 2) * (point[0] - width / 2) - point[1])


def show_annotated_image(img, boxes, labels, scores, class_ids, score_threshold=0.0, area_threshold=1,
                         default_boxes=None, transform_corners=None,
                         max_dets=None, showfig=False, image_id=None,
                         lower=True, max_parabola_deviation=None):
    image_area = get_image_area(img)
    rel_areas = [get_box_area(box) / image_area for box in boxes]
    # good_ids = torch.nonzero(torch.logical_and(
    #     scores.float() > score_threshold,
    #     torch.tensor(rel_areas) < area_threshold)
    # ).view(-1)
    good_ids = torch.nonzero(torch.logical_and(
        scores.float() > score_threshold,
        torch.tensor(rel_areas).cuda() < area_threshold)
    ).view(-1)
    if good_ids.numel() > 0:
    ...
    if lower:
        center_point = [img.width / 2, img.height]
    else:
        center_point = [img.width / 2, 0]

    # Finding center point for parabola
    # We need only its y coordinate as y0
    closest = None
    closest_dist = None
    for i in range(len(boxes)):
        box_center = get_center(boxes[i])
        dist = sqr_dist_between(center_point, box_center)
        if closest is None or dist < closest_dist:
            closest = box_center
            closest_dist = dist

    if max_parabola_deviation is not None:
        # Solving equation for parabola
        # y = k * (x - width / 2) ^ 2 + y0
        y0 = closest[1]

        if lower:
            k = -y0 * 1.0 / ((img.width / 2) * (img.width / 2))
        else:
            k = (img.height - y0) * 1.0 / ((img.width / 2) * (img.width / 2))

        distances = [approx_dist_between_point_and_parabola(k, y0, img.width, get_center(box)) for box in boxes]
        boxes_ids = np.argsort(distances)
        close_boxes_num = len(list(filter(lambda x: x < (img.width + img.height) * max_parabola_deviation, distances)))
        boxes_ids = boxes_ids[:close_boxes_num]
        boxes = boxes[boxes_ids]
