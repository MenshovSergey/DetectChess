...
class Os2dBoxCoder:
    ...
    def __init__(self, positive_iou_threshold, negative_iou_threshold,
        ...
        self.do_nms_across_classes = False
    ...
    @staticmethod
    def _nms_box_lists(boxlists, nms_iou_threshold, area_threshold=1, image_area=1000000):
        boxes = cat_boxlist(boxlists)
        scores = boxes.get_field("scores")
    
        ids_boxes_keep = nms(boxes, nms_iou_threshold, area_threshold=area_threshold, image_area=image_area)
    ...
    def decode_pyramid(self, loc_scores_pyramid, cls_scores_pyramid, img_size_pyramid, class_ids,
               nms_score_threshold=0.0, nms_iou_threshold=0.3, area_threshold=1, image_area=1000000,
        ...
        for real_label in set(class_ids):
            ...
            if len(masked_boxes_pyramid) > 0:
                boxes_after_nms = self._nms_box_lists(masked_boxes_pyramid, nms_iou_threshold, area_threshold=area_threshold, image_area=image_area)
                boxes_per_label.append(boxes_after_nms)
            
        if self.do_nms_across_classes:
            boxes_stacked = \
                self._nms_box_lists(boxes_per_label, nms_iou_threshold, area_threshold=area_threshold, image_area=image_area)
        else:
